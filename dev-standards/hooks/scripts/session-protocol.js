...
- The contract doc wins. When a named contract document (DECISIONS.md, a manifest, a spec) and the conversation disagree, follow the document, say so, and stop.
- Log assumptions, don't stall. When the record has a gap, make a reasonable call and log it to ASSUMPTIONS.md (format: project-artifacts skill). At session end every assumption gets a disposition: promoted, refuted, or carried.
- EXCEPT — stop and ask a human before any action that would:
  ...
  (5) invalidate a settled decision in DECISIONS.md — filling a genuine gap is what the assumptions log is for;
